...
export default function EditEstimatePage() {
      ...
      if (data.success) {
        ...
        if (est.items) {
          try {
            ...
            console.warn('Items parse error, using default:', e);
            parsedItems = [
              { id: 1, description: '', quantity: 1, price: 0, discount: 0 },
            ];
          }
        ...
        setFormData({
          ...
          items: parsedItems,
          notes: est.notes || '',
          terms: est.terms || 'Payment due within 30 days.',
          taxRate: parseFloat(est.taxRate) || 10,
  ...
  const calculateTotals = () => {
    if (!formData) return { subtotal: 0, taxAmount: 0, total: 0 };
    let subtotal = 0;
  ...
  if (!formData)
    return (
      <div className="min-h-screen bg-[#0F172A] flex items-center justify-center">
        <p className="text-[#94A3B8]">Estimate not found</p>
      </div>
    );
